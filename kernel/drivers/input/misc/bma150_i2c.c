...
unsigned long		bma_delay=10;
//timer=50 ms changed by qliu
...
//****************************************************************************
    ...
    do {
			...
			BDBG( "bma150_verify_min_max: min_max_ok  %d\n", min_max_ok);
			...
			//xxBDBG( "orientation.x%d avg.x%d,&offset_x:%d, &step.x:%d, &min_accel_adapt_x:%d, &min_offset_x:%d, &min_reached_x:%d, &prev_steps_x:%d", orientation.x,avg.x,offset_x,step.x,min_accel_adapt_x,min_offset_x,min_reached_x,prev_steps_x);
			BDBG( "dummy: %d\n", dummy);
...
static int bma150_i2c_calibrate(int arg)
{
	...
	if(rc != 0)
	{
		BDBG(KERN_WARNING "bma150_calibration failed %d",rc);
		return rc;
	}
...
//	struct bma150_platform_data *pdata = i2c->dev.platform_data;
	...
	//printk("bma150_i2c_probe\n");
	//printk("%x==================================\n",i2c->addr);
	//printk("i2c->adapter.name is %s\n",i2c->adapter->name);
	//printk("i2c->adapter.nr is %d===============\n",i2c->adapter->nr);
...
static int bma150_i2c_suspend(struct i2c_client *i2c, pm_message_t mesg)
{
	...
	rc = bma150_i2c_read(bma150_driver_data, 0x0A, &buf[1], 1);
	buf[1] = buf[1] | 0x01;     //sleep
	buf[0] = 0x0A;
	rc += bma150_i2c_write(bma150_driver_data, buf, 2);
	printk("bma150_i2c_suspend sleep rc= %d\n", rc);
...
static int bma150_i2c_resume(struct i2c_client *i2c)
{
	...
	//set sleep bit = 0
	rc += bma150_i2c_read(bma150_driver_data,BMA150_REG_CONTROL_0A, &buf[1], 1);
	buf[0] = BMA150_REG_CONTROL_0A;
	buf[1] = buf[1] & ~0x01;
	rc += bma150_i2c_write(bma150_driver_data, buf, 2);
	printk("bma150_i2c_resume set sleep bit = 0 rc= %d\n", rc);
	
	//set wakeup bit = 0
	rc = bma150_i2c_read(bma150_driver_data, BMA150_REG_CONTROL_15, &buf[1], 1);  
	buf[0] = BMA150_REG_CONTROL_15;
	buf[1] = buf[1] & ~0x01;
	rc += bma150_i2c_write(bma150_driver_data, buf, 2);
	printk("bma150_i2c_resume set wakeup bit = 0 rc= %d\n", rc);
